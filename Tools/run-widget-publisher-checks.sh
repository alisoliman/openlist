...
OUT=$(mktemp -d)
trap 'rm -rf "$OUT"' EXIT
xcrun swiftc -swift-version 6 -default-isolation MainActor -o "$OUT/widget-publisher-checks" \
  openlist/Model/*.swift Shared/ListAccent.swift Shared/ReviewSession.swift Shared/WidgetSnapshot.swift Shared/AppGroup.swift openlist/Next/NextEditorTypography.swift \
  ...
  openlist/Services/ListTasksProjection.swift openlist/Services/ActiveTaskPolicy.swift openlist/Services/WidgetSnapshotPublisher.swift \
  Tools/EditorChecks/Support.swift Tools/WidgetPublisherChecks/main.swift
"$OUT/widget-publisher-checks"
