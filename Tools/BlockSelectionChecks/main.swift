...
let document = UUID(), otherDocument = UUID()
var controller = BlockSelection()
var selected = controller.select(ids[1], in: document, visible: ids, selected: [])
check(selected == [ids[1]] && controller.scopeID == document, "Editing a line selects it alone, in its document's scope")
...
check(controller.reconcile(in: otherDocument, visible: [ids[0]], selected: selected) == selected,
      "Another document's updates cannot destroy the active selection")
selected = controller.select(ids[0], in: otherDocument, visible: [ids[0]], selected: selected)
check(selected == [ids[0]] && controller.scopeID == otherDocument, "Changing documents starts a new visible scope")
selected = controller.reconcile(in: otherDocument, visible: [], selected: selected)
check(selected.isEmpty && controller.visibleIDs.isEmpty, "An empty filtered view clears hidden selection")
controller.clear()
check(controller.scopeID == nil && controller.visibleIDs.isEmpty, "Clearing forgets the scope and its rows")
