...
OUT=$(mktemp -d)
trap 'rm -rf "$OUT"' EXIT
xcrun swiftc -swift-version 6 -default-isolation MainActor -o "$OUT/widget-snapshot-checks" \
  openlist/Model/*.swift Shared/ListAccent.swift Shared/WidgetSnapshot.swift Shared/WidgetKind.swift Tools/WidgetSnapshotChecks/ReviewSession.swift openlist/Next/NextEditorTypography.swift \
