...
check(coordinator.plan.blocks.first { $0.isActive }?.end == date(14, 10), "second overrun adds another 15-minute increment")
coordinator.tick(now: date(14, 10, 7), checkClockGap: false)
check(coordinator.activeSession == nil, "meeting automatically pauses work")
check(store.workSessions(taskID: a.id).first?.endedAt == date(14, 10), "late timer records exact meeting boundary, not observed callback time")
...
check(!coordinator.start(task: a, now: date(14, 10, 10)), "Start cannot cross fixed external busy time")
...
check(coordinator.activeSession == nil, "away opt-in still respects end of availability / lunch break")
...
check(recovered.start(task: freeTask, now: date(16, 13)), "archived task's retained pin cannot block a visually free slot")
...
func checkSchedulingNudges() throws {
    ...
    do {
        ...
        check(planner.activeSession == nil && fixtureStore.workSessions(taskID: first.id).first?.endedAt == date(14, 9, 45), "a further displacement pauses exactly at the approved boundary")
        check(planner.overrunNudge?.needsConfirmation == true && planner.overrunNudge?.movedTaskCount == 1, "another affected task requires explicit continued-work approval")
        check(planner.plan.blocks.first { $0.taskID == next.id }?.start == nextAnchor.start, "waiting for confirmation preserves the next task's placement")
        planner.tick(now: date(14, 9, 46), checkClockGap: false)
        check(planner.overrunNudge?.needsConfirmation == true && planner.trackedMinutes(for: first, now: date(14, 9, 46)) == 45, "an unanswered confirmation persists without tracking the waiting minute")
        planner.acceptMoreTime(now: date(14, 9, 46))
        check(planner.activeSession?.startedAt == date(14, 9, 46), "Continue starts a fresh actual work interval")
        check(planner.plan.blocks.first { $0.isActive }?.end == date(14, 10, 1), "explicit confirmation grants exactly 15 minutes from the response")
        check(planner.overrunNudge == nil && planner.plan.blocks.first { $0.taskID == next.id }?.start == date(14, 10, 1), "accepted extension clears confirmation and reflows affected work")
    ...
    do {
        ...
        check(planner.activeSession == nil && planner.overrunNudge?.needsConfirmation == true, "editing an estimate never resets the already consumed automatic extension")
        check(planner.overrunNudge?.estimatedEnd == date(14, 10) && planner.overrunNudge?.proposedEnd == date(14, 10, 15), "confirmation describes exactly the next 15-minute interval")
    }
    do {
        let (fixtureStore, planner, lifetime) = try fixture()
        defer { withExtendedLifetime(lifetime) {} }
    ...
    do {
        ...
        check(planner.activeSession == nil && fixtureStore.workSessions(taskID: solo.id).first?.endedAt == date(14, 10), "even free extensions never cross a meeting boundary")
        check(planner.overrunNudge == nil && planner.resumeTaskID == solo.id, "a fixed boundary offers resume after the meeting instead of extension approval")
    }
    do {
    ...
    do {
        ...
        check(fixtureStore.workSessions(taskID: first.id).first?.endedAt == date(14, 9, 35), "a late Pause or lock event cannot record past the known fixed boundary")
    }
    do {
        let (fixtureStore, planner, lifetime) = try fixture()
        defer { withExtendedLifetime(lifetime) {} }
        let first = add("Edit a pending confirmation estimate", to: fixtureStore, priority: 3)
        _ = add("Keep the other work", to: fixtureStore, priority: 2)
        planner.bootstrap(now: date(), monitorsEnabled: false)
        check(planner.start(task: first, now: date()), "pending estimate fixture starts")
        planner.tick(now: date(14, 9, 45), checkClockGap: false)
        check(planner.overrunNudge?.needsConfirmation == true, "pending estimate fixture waits after the first extension")
        first.schedulingEstimateMinutes = 90
        fixtureStore.save()
        planner.storeDidChange(now: date(14, 9, 46))
        planner.acceptMoreTime(now: date(14, 9, 46))
        check(planner.plan.blocks.first { $0.isActive }?.end == date(14, 10, 1), "Continue grants exactly 15 minutes even if the estimate changed while paused")
    }
    do {
        let (fixtureStore, planner, lifetime) = try fixture()
        defer { withExtendedLifetime(lifetime) {} }
        ...
        check(fixtureStore.workSessions(taskID: first.id).first?.endedAt == date(14, 10, 15), "a late callback still clips to the newly established fixed boundary")
    ...
    do {
        ...
        _ = add("One automatic displacement allowed", to: fixtureStore, priority: 2)
        planner.bootstrap(now: date(), monitorsEnabled: false)
    ...
    do {
        ...
        fixtureStore.save()
        planner.storeDidChange(now: date(14, 9, 10))
        check(working().first?.start == date() && working().first?.end == date(14, 10, 5), "a replan while working keeps the time already worked on the calendar")
