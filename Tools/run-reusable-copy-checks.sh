...
xcrun swiftc -swift-version 6 -default-isolation MainActor -o "$OUT/copy-checks" \
  ...
  openlist/Services/Store.swift openlist/Services/Store+Blocks.swift openlist/Services/Store+Copies.swift openlist/Services/Store+Sync.swift \
  ...
  Tools/EditorChecks/Support.swift Tools/ReusableCopyChecks/main.swift
"$OUT/copy-checks" "$OUT/Copies.store" write
"$OUT/copy-checks" "$OUT/Copies.store" reopen
