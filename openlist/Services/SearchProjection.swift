...
nonisolated struct SearchProjection: Sendable {
    ...
    init(corpus: SearchCorpus, options: SearchOptions) throws {
        ...
        let matchingLists = try lists.enumerated().filter { offset, list in
            if offset.isMultiple(of: 128) { try Task.checkCancellation() }
            return list.mergedIntoID == nil
        ...
        result += matchingLists.map { list in
        ...
        for (offset, block) in matchingBlocks.enumerated() {
