...
- The Work panel, Work history, the inspector's Full history, Settings › Data and
  Notifications, native extras, write a moment as the pills and the Calendar do: "Today
  10:00", "Fri 25 10:00", with the year only when it isn't this one (`MomentText`); a
  planned slot reads "Today 10:00–11:30 · 90 min", and the hours editor names an
  override's day as the pills do. Work history words a session's pause as the Work panel
  does, and the panel's completion reads "Done", a repeat's "Rolls to Wed 30", as the tray
  does.
